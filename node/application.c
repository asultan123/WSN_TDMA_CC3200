...
void APP_node()
{
    ...
    do
    {
        UART_PRINT("\r\nSENDING SYSTEM PARAM REQUEST TO SINK\r\n");
        ...
        if(IP_send_data(&gateway, appBuff, 2) == IP_SEND_SUCCESS)
        {
            ...
            if(IP_recv_data(&srcIp, &respData, 100) == IP_MSG_APPLICATION)
            {
                ...
                systime = UTIL_convert_8Bytes_to_unsigned_long_long(respData+4) + 100*retries + 75;

                UART_PRINT("\r\nPARAMS RECIEVED, SYSTIME: %llu, SYSPERIOD: %d\r\n",systime, sysPeriod);
                break;
            }
        }
    ...
    while(!UTIL_within_tolerance(systime%sysPeriod, 1))
    {
        //SLEEP HERE
    }
    ...
    while(1)
    {
        ...
        UART_PRINT("\r\nSTART OF PERIOD AT SYSTIME: %llu\r\n",systime);
        ...
        UART_PRINT("\r\nSENDING DATA AT SYSTIME: %llu\r\n", now);

        if(IP_send_data(&gateway, appBuff, 2) == IP_SEND_SUCCESS)
        {
            UART_PRINT("\r\nSENT DATA TO SINK AT SYSTIME: %llu\r\n", systime);
        }
...
void APP_sink()
{
    ...
    while(1)
    {
        ...
        if(res == IP_MSG_APPLICATION){
            ...
            UART_PRINT("\r\nAPPLICATION MESSAGE RECIEVED: %s", recvData);
            UART_PRINT(" FROM SRC IP: ");
            UTIL_UART_print_ip(recvIp);
            UART_PRINT("\r\n");
